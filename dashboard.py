...
function bestScores(p) {
  // {win, draw} scorelines: the most likely decisive result and the most
  // likely draw. Uses outcome_scores when present, else derives them from the
  // stored top-5 (older frozen predictions), else falls back to the modal.
  const os = p.outcome_scores;
  if (os && os.draw && (os.team1 || os.team2)) {
    return {win: (p.p_win >= p.p_loss) ? os.team1 : os.team2, draw: os.draw};
  }
  if (p.top_scores) {
    let win = null, draw = null;
    for (const t of p.top_scores) {
      const parts = t.score.split('-'), a = +parts[0], b = +parts[1];
      if (a === b) { if (!draw) draw = t; } else if (!win) { win = t; }
      if (win && draw) break;
    }
    if (win) return {win: win, draw: draw};
  }
  return {win: {score: p.modal_score, p: p.modal_p}, draw: null};
}

function scoreFor(score, m) {
  // Stored score is "a-b" = team1 a, team2 b. Present it winner-first with
  // the team named, so it is unambiguous who the result favours.
  const parts = String(score).split('-');
  const a = +parts[0], b = +parts[1];
  if (a > b) return esc(m.team1) + ' ' + a + '-' + b;
  if (b > a) return esc(m.team2) + ' ' + b + '-' + a;
  return 'draw ' + a + '-' + b;
}
...
function callLine(m) {
  ...
  // Read everything from the recommended team's perspective, with each
  // probability labelled by team, so it never reads in reverse.
  let lead, others;
  if (pick === 'team1') {
    lead = '<b>' + esc(m.team1) + ' to win ' + pct(p.p_win) + '</b>';
    others = 'draw ' + pct(p.p_draw) + ', ' + esc(m.team2) + ' win ' +
      pct(p.p_loss);
  } else if (pick === 'team2') {
    lead = '<b>' + esc(m.team2) + ' to win ' + pct(p.p_loss) + '</b>';
    others = 'draw ' + pct(p.p_draw) + ', ' + esc(m.team1) + ' win ' +
      pct(p.p_win);
  } else {
    lead = '<b>Draw most likely, ' + pct(p.p_draw) + '</b>';
    others = esc(m.team1) + ' win ' + pct(p.p_win) + ', ' + esc(m.team2) +
      ' win ' + pct(p.p_loss);
  }
  // Always show two scorelines: the most likely decisive result and the most
  // likely draw, each with fair odds, so a draw is always represented and a
  // winner is always shown.
  const bs = bestScores(p);
  const odds = function (o) {
    return ' (' + pct(o.p) + ', fair ' + fairOdds(o.p) + ')';
  };
  let scoreText = 'Most likely ' + scoreFor(bs.win.score, m) + odds(bs.win);
  if (bs.draw) {
    scoreText += ', or a ' + bs.draw.score + ' draw' + odds(bs.draw);
  }
  let s = lead + ' (' + others + '). ' + scoreText;
  if (p.p_advance !== undefined && m.status !== 'played') {
    ...
    s += '; <b>' + esc(fav) + ' to advance ' + pct(fp) + '</b>';
  }
  return s;
}
